...
// globals
unsigned int k = 0;           // number of hi hash functions
...
int main(int argc, char **argv) {
    ...
    do {
        ...
        */
        ...
        vector<Result *> *approximateResult = nns.calculate_approximate_NN(N);
        cout << "Done!" << endl;
        ...
        for (int i = 0; i < exactResult->size() && i < approximateResult->size() && i < range_search_res->size(); i++) {   // for each query point
            ...
            for (int j = 0; j < apprx_res->NNs->size(); j++) {
                ...
                output_stream << "distanceLSH: " << approx_nn.dist << endl;
                output_stream << "distanceTrue: " << real_nn.dist << endl;
            }
            output_stream << "tLSH: " << apprx_res->dt << " micro secs" << endl;
